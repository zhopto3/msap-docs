...
def apply_grammar(head: conllu.Token, children: List[conllu.Token]):
    ...
    if TAM_nodes:
        ...
        # TODO: aggiungere diatesi e aspetto?

    # if there are cases or conjunctions "consume" them as well
    ...
    relation_nodes = [child for child in children if
                      ...
                      and (child['upos'] != 'PART' or child['lemma'] == "'s")] # TODO: arianna inventa la lista di lemmi
    if relation_nodes:
    ...
    if verb:
        ...
        # treat determiners
        det_nodes = [child for child in children if child['deprel'] == 'det'] #TODO: controlla det:predet
        if det_nodes:
            assert len(det_nodes) == 1 # there should only be one determiner #TODO: cambia per l'italiano
            det_node = det_nodes[0]
            children = [node for node in children if node != det_node]
            # TODO: for feat_name, feat_value in determiners[det_node['lemma']]:
            for feat in determiners[det_node['lemma']]: # set the appropriate features to the head using the det_feats map
                if feat in head['ms feats']:
                    det_feats = head['ms feats'][feat]
                    head['ms feats'][det_feats[0]] = det_feats[1]
                    if len(det_feats) == 4: # TODO: controllare quando mai è lungo 4
